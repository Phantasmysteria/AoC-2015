...
data = {}
data["00-raw"] = aocutils.listLines(day="15", part="2")
...
data["01-parsed"] = np.array([[int(item.group(2)), int(item.group(3)), int(item.group(4)), int(item.group(5)), int(item.group(6))] for item in data["01-parsed"]])
...
# exclude calories from score counting
data["06-scores"] = np.prod(data["05-propertySums"][:-1, :], axis=0)

data["07-scoresOfCookiesWith500Calories"] = data["06-scores"][data["05-propertySums"][-1] == 500]

output = np.max(data["07-scoresOfCookiesWith500Calories"])
print(output)
